...
#FILES=$(find ./modules -type f -name "*.adoc")
...
if [ -n "${FILES}" ] ;
    then
        ...
        echo ""
        for FILE in ${FILES}
            do vale $FILE --filter='.Name == "AsciiDoc.ValidConditions"' --output=line --no-exit
            #do vale $FILE --minAlertLevel=error --output=line --no-exit
        done
    else
